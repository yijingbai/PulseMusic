...
class InterfaceController: WKInterfaceController, HKWorkoutSessionDelegate,WCSessionDelegate {
    ...
    func heartRateReplyHandler(res : [String : AnyObject]) {
        NSLog("Reply received")
    }
    
    func heartRateErrorHandler(e : NSError) {
        NSLog("Reply error")
    }
    ...
    func updateHeartRate(samples: [HKSample]?) {
        ...
        dispatch_async(dispatch_get_main_queue()) {
            ...
            NSLog("Start communicate with iphone")
            self.watchSession = WCSession.defaultSession()
            self.watchSession?.sendMessage(
                    ["rate" : String(UInt16(value))],
                    replyHandler: self.heartRateReplyHandler,
                    errorHandler: self.heartRateErrorHandler
                )
            NSLog("Start send rate: " + String(UInt16(value)))
